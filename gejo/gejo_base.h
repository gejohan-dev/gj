...
#define SwapVar(type, x, y) do {##type __tmp = x; x = y; y = __tmp;} while(false)
#define SwapArray(array, type, i, j) do {##type __tmp = array[i]; array[i] = array[j]; array[j] = __tmp;} while(false)
